...
static byte waitForAck() {
    ...
    while (!ackTimer.poll(ACK_TIME)) {
        if (rf12_recvDone() && rf12_crc == 0 && rf12_hdr == (RF12_HDR_CTL | RF12_HDR_DST |config.nodeId))
            return 1;
        //set_sleep_mode(SLEEP_MODE_IDLE);
        //sleep_mode();
        //Serial.println(rf12_hdr);
        //Serial.println(RF12_HDR_CTL | RF12_HDR_DST | config.nodeId);
...
void doSend(){
        ...
        for (byte i = 0; i < RETRY_LIMIT; ++i) {
            ...
            Serial.println(" b");
            byte header = RF12_HDR_ACK | dest; // = cmd == 'a' ? RF12_HDR_ACK : 0;
            //if (dest)
            //    header |= RF12_HDR_DST | dest;
            //Serial.println(header);
                
            rf12_sendStart(header, testbuf, sendLen, RADIO_SYNC_MODE);
